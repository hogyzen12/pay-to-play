...
export const fetchPCIKeys = async () => {
  try {
    ...
    const result = await axios.get(
      `${circleAPIBase}/v1/encryption/public`,
      config,
    );
...
export const makePayment = async paymentParams => {
  try {
    ...
    let cardResult = await axios.post(
      `${circleAPIBase}/v1/cards`,
      cardDetails,
      config,
    );
...
export const checkPaymentStatus = async paymentId => {
  try {
    ...
    let paymentStatusResult = await axios.get(
      `${circleAPIBase}/v1/payments/${paymentId}`,
      config,
    );
    /**
  ...
  try {
    ...
    if (!result.status) {
      return {
        status: false,
        message:
          'Please check the sender account balance, if it has SOL balance to transfer.',
      };
    }
